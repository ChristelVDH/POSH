...
Function GPOSize($objGPO){
	...
	write-verbose "the size for $objGPO.DisplayName is $Result"
	return $Result
}
}# end begin
...
$script:GPObj | sort size
}

# one liner to get reports for all GPO objects in domain
# Get-GPO -All | %{Get-GPOReport -guid $_.id -ReportType html -Path (join-path -path "E:\Debug\GPO\reports" -childpath "$($_.displayname).html")}
